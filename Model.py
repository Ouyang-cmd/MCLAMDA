...
class MCLAMDA(nn.Module):
    ...
    def forward(self, graph, diseases, mirnas):
        ...
        feature_only = feature.detach().numpy()
        ...
        X_embedded = tsne.fit_transform(feature_only[:591])

        plt.figure()
        plt.scatter(X_embedded[:, 0], X_embedded[:, 1], c='skyblue', marker='o', edgecolors='k', alpha=0.7)
        print(f'Number of points in final_disease_{timestamp}.png: {len(X_embedded)}')
        plt.savefig(os.path.join(save_path, f"final_disease_{timestamp}.png"))
        plt.show()

        tsne = TSNE(n_components=2, random_state=42)
        Y_embedded = tsne.fit_transform(feature_only[591:])
        print(f'Number of points in final_miRNA_{timestamp}.png: {len(Y_embedded)}')
        plt.figure()
        plt.scatter(Y_embedded[:, 0], Y_embedded[:, 1], c='lightcoral', marker='o', edgecolors='k', alpha=0.7)
        plt.savefig(os.path.join(save_path, f"final_miRNA_{timestamp}.png"))
        plt.show()
...
class FeatureExtractor(nn.Module):
    ...
    def init_embedding_and_attention(self):
        ...
        self.attention = {
            'disease': nn.MultiheadAttention(self.embedding_size, num_heads=2).to(self.ctx),
            'mirna': nn.MultiheadAttention(self.embedding_size, num_heads=2).to(self.ctx)
        }
